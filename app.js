...
app.use(function (req, res, next) {
    ...
    jwt.verify(req.headers.authorization, pem, { algorithms: ['RS256'] }, function(err, decodedToken) {
...
app.get('/getMyLeagues', function (req, res) {
    ...
    var returnArray = []
    con.query("Select * from userleagueteam, dotaleague WHERE userleagueteam.user = ? AND dotaleague.leagueid = userleagueteam.leagueid", [req.query.user], async function (err, result) {
        if (err) throw err;
        // console.log("Result: " + result);
        ...
        for (var x = 0; x < result.length; x++) {
            ...
            console.log(total + " ANND  " + position)
...
app.get('/getTournamentsGrouped', async function (req, res) {
...
app.get('/getTournaments', async function (req, res) {
    //res.setHeader('Access-Control-Allow-Origin', 'http://localhost:3000');
    //res.setHeader('Access-Control-Allow-Credentials', true);
...
app.get('/getPlayersNotFound', async function (req, res) {
    var username = await getUserName(req.headers.authorization)
    console.log("USERNAME "+ username)
    if (username != "matt") {
        return
...
app.get('/getLeagueShowLocks', async function (req, res) {
    var username = await getUserName(req.headers.authorization)
    console.log("USERNAME "+ username)
    if (username != "matt") {
        return
            ...
            for (var i = 0; i < result.length; i++) {
                ...
                for (var j = 0; j < playerResult.length; j++) {
                    if (playerResult[j]['teamname'] == result[i]['teamname']) {
                        ...
                        playerArray.push({
                            "playername": playerResult[j]['playername'],
    ...
    for (let i = 0; i < collection.length; i++) {
        ...
        for (let j = 0; j < 11; j += 2) {
            if (j > 0) {
                ...
                team.push({
                    "playername": temporaryDotaTeam[j + notesOffset].innerHTML,
...
app.get('/getUserLeagueTeam', async function (req, res) {
    ...
    con.query("Select * from userleagueteam WHERE user = ? and leagueid =? ", [username, req.query.leagueid], function (err, result) {
        if (err) throw err;
            ...
            [req.query.leagueid, result[0].player1, result[0].player2, result[0].player3, result[0].player4, result[0].player5], function (err, playerResult) {
                ...
                for (var x = 0; x < playerResult.length; x++) {
                    ...
                    selectedPlayers.push({
                        "playername": playerResult[x]['playername'],
...
app.get('/getLeagueInfo', function (req, res) {
    // res.send({"dsa":'Hello world from Express.'});
    con.query("Select * from  dotaleague WHERE leagueid = ?", [req.query.leagueid], function (err, result) {
        if (err) throw err;
...
app.get('/getLeagueRosterLocks', function (req, res) {
    ...
    con.query("Select * from  dotaleague WHERE leagueid = ?", [req.query.leagueid], function (err, result) {
        ...
        var returnJson = {
            "lockStart": result[0].rosterlockstart, "lockEnd": result[0].rosterlockend,
            ...
            con.query("Select * from leagueteams WHERE leagueid=? AND (teamid=? OR teamid = ?)", [leagueid, res[x].radiant_team_id, res[x].dire_team_id], async function (err, result) {
                ...
                if (result.length == 2) {
                    ...
                    for (var y = 0; y < matchRes.players.length; y++) {
                        ...
                        matchesPlayers.push({
                            "playerid": matchRes.players[y].account_id,
